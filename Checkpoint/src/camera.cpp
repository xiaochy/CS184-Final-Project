...
void Camera::ArcBallRotation(int x, int y)
{
    ...
    if (dotprod != 1)
    {
        ...
        Matrix3f rotate3D;
        rotate3D = AngleAxisf(angle, axis);
        mCurrentRot = Matrix4f::Identity();
        mCurrentRot.block<3, 3>(0, 0) = rotate3D;
...
void Camera::PlaneTranslation(int x, int y)
{
    ...
    float d = float(mViewport[3]) / 2.0f /
              tan(mPerspective[0] * M_PI / 180.0f / 2.0f);
    ...
    mCurrentCenter = mStartCenter +
                     +move[0] * Vector3f(mCurrentRot(0, 0), mCurrentRot(0, 1),
                                         mCurrentRot(0, 2)) +
                     move[1] * Vector3f(mCurrentRot(1, 0), mCurrentRot(1, 1),
                                        mCurrentRot(1, 2));
}
...
{
    // back up distance
    gluLookAt(0, 0, mCurrentDistance, 0, 0, 0, 0.0, 1.0, 0.0);
